...
class LLMRouter:
    async def route_call(self, task_type: str, prompt: str, system_prompt: str = "") -> Optional[str]:
        ...
        - classification: Ollama -> Groq (Llama-70b) -> Gemini Pro
        - reasoning / generation: Ollama -> Groq (Llama-70b) -> Gemini Pro
        """
        response = ""
        if task_type == "classification":
            logger.info("[LLM Router] Attempting classification with Ollama...")
            response = await ollama_adapter.generate(prompt, system_prompt)

            if not response:
                logger.info("[LLM Router] Ollama unavailable. Failing over to Groq...")
            ...
            logger.info("[LLM Router] Attempting reasoning/generation with Ollama...")
            response = await ollama_adapter.generate(prompt, system_prompt)
